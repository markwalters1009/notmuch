...
;; the following is to allow testing of the various no-body options
(defvar notmuch-pick-headers-hack "--output-body=false")
;;(defvar notmuch-pick-headers-hack "--format=json")
...
;;FIXME comment all the variables
;; CHECK is this actually a thread-id?
...
(defun notmuch-pick-refresh-result ()
  (let ((msg (notmuch-pick-get-message-properties))
	(inhibit-read-only t))
    (beginning-of-line)
...
;; This used to check for (not (window-full-height-p
;; notmuch-pick-message-window))) Why?
