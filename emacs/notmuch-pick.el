...
(defun notmuch-pick-show-match-message-with-wait ()
  ...
  (while (and (not (notmuch-pick-get-match))
	      (not (string= notmuch-pick-process-filter-data "\n]")))
  ...
  (let ((buffer (process-buffer proc))
	...
	(if (buffer-live-p buffer)
		...
		(let ((inhibit-read-only t)
		  ...
		  (when (eq status 'exit)
		    (if (and (not (string= notmuch-pick-process-filter-data "\n]"))
			     (not (string= notmuch-pick-process-filter-data "[\n]")))
		...
		(let ((frame (match-string 0 string))
		  ...
		  (when (or (= (elt frame 1) ?\[) (= (elt frame 1) ?,))
			...
			(let ((json-object-type 'plist)
			  ...
			  (backward-char)
			  (setq json (json-read))))
		      (setq json-batch (append json-batch (list json)))))
		  (setq string (substring string (- frame-end 1)))))
	      (goto-char (point-max))
	      (notmuch-pick-insert-forest json-batch)
